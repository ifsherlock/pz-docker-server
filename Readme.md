...
* 🔑 服务端安全配置:
  ...
  * 点击“保存并重启”后，玩家密码写入 `servertest.ini`，管理员凭据同步到 Build 42 的服务器数据库并传给启动参数。
...
* 服务端配置分组: 面板已将 146 个配置项细分为服务端基础、网络与连接、聊天与语音、游戏规则、存档与备份、地图、玩家与 PVP、世界环境、车辆、客户端限制、反作弊、Discord、模组与工坊和服务端安全，密码相关项固定在最后。
